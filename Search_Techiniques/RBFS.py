...
def recursive_gbfs(current_node, goal, path, total_cost, total_heuristic_cost):
    ...
    total_heuristic_cost += current_node.heuristic_cost
    print(f"Path: {path}")
...
# Define the graph for the Romania map
...
rimnicu_vilcea = Node("RimnicuVilcea", 193)
...
if recursive_gbfs(start_node, goal_node.name, path, total_cost, total_heuristic_cost):
